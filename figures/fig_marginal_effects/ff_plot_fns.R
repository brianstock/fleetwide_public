...
ff_plot_factor <- function(dat,mod.rf,ff,x,covar){
	...
	g <- ggplot(pred.df.plot, aes(x = variable, y = value)) +
	        ...
	        theme_classic() +
	        ylab("Feature contribution") +xlab(x)# + xlab("") + theme(axis.text.x = element_blank(), axis.text.y = element_blank())
...
ff_plot_cont <- function(dat,mod.rf,ff,thecov,covar){
	...
	g <- ggplot(df, aes(x=x,y=med)) +
	        ...
	        coord_cartesian(ylim = c(minY,maxY)) +	        
	        theme_classic() +
	        ylab("Feature contribution") + xlab(thecov)# + xlab("") + theme(axis.text.x = element_blank(), axis.text.y = element_blank())
	return(g)
}
