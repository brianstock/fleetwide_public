...
rf_correct_bcs = function(formula, combined_data, iter=2) {
  ...
  for(i in 1:iter) {
    rf = cforest(as.formula(formula), data = train_data, 
      control = cforest_control(ntree=1000, mtry=4))
    train_data$y_i <- train_oob_pred[,i+1] <- as.vector(predict(rf)) - train_oob_pred[,i]
...
legend('topright', c("Bias", "MSE"), lty = 1:2, pch = c(1, 19), box.lwd =NA)
