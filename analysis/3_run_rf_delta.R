...
for(i in 1:length(spp)) {
  ...
  for(h in 1:length(pct)) {
    ...
    for(j in 1:n_sims) {
      ...
      results.sim = data.frame(model = rep("rfdelta",4),
      ...
      rf_binomial = randomForest(as.factor(presence) ~ avg_lat + avg_long + I(avg_lat^2) + 
          I(avg_long^2) + year + depth_interval + bimonth + logret, 
        data = training_data, ntree=1000)
      rf_pos = randomForest(response ~ avg_lat + avg_long + I(avg_lat^2) + 
          I(avg_long^2) + year + depth_interval + bimonth + logret, 
        data = training_data[which(training_data$response>0),], ntree=1000)      
      
      test_data$rf.pred <- predict(rf_binomial, newdata = test_data, type="prob")[,2] * predict(rf_pos, newdata = test_data, type="response")
      ...
      rf_binomial = randomForest(as.factor(presence) ~ avg_lat + avg_long + I(avg_lat^2) + 
          I(avg_long^2) + year + depth_interval + bimonth, 
        data = training_data, ntree=1000)
      rf_pos = randomForest(response ~ avg_lat + avg_long + I(avg_lat^2) + 
          I(avg_long^2) + year + depth_interval + bimonth, 
        data = training_data[which(training_data$response>0),], ntree=1000)      
      ...
      rf_binomial = randomForest(as.factor(presence) ~ year + depth_interval + bimonth + logret, 
        data = training_data, ntree=1000)
      rf_pos = randomForest(response ~ year + depth_interval + bimonth + logret, 
        data = training_data[which(training_data$response>0),], ntree=1000)      
      ...
      rf_binomial = randomForest(as.factor(presence) ~ year + depth_interval +bimonth, 
        data = training_data, ntree=1000)
      rf_pos = randomForest(response ~ year + depth_interval + bimonth, 
        data = training_data[which(training_data$response>0),], ntree=1000)      
    ...
    # save output
    saveRDS(results, file = paste0("results/byyear_rfdelta_",spp[i],"_",pct[h],".rds"))
  }
