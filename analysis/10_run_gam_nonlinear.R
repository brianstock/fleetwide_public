# Brian Stock
# Sept 2018
# GAM fit covariates with *full non-linear flexibility* (not as factors):
#   gam(response ~ s(avg_lat, avg_long, k=50) + s(year,k=5) + s(depth,k=5) + s(julian_day,k=5) + s(time,k=5) + gear + logret)

# source("/home/brian/Documents/Bycatch/fleetwide/analysis/10_run_gam_nonlinear.R")
...
foreach(i=1:length(spp), .packages=c("tidyverse","mgcv")) %dopar% {
  ...
  for(h in 1:length(pct)) {
    ...
    for(j in 1:n_sims) {
      ...
      results.sim = data.frame(model = rep("gamnonlinear",1),
                          spatial = c("yes"),
                          effort = c("yes"),
                          sim = rep(j,1),
	  ...
	  mod = gam(response ~ s(avg_lat, avg_long, k=50) + s(year,k=5) + s(depth,k=5) + s(julian_day,k=5) + s(time,k=5) + gear + logret, data=training_data, family=Tweedie(p=tweedie_p[i]), discrete=TRUE, method="REML") # 1 min
      test_data$mod.pred <- predict(mod, newdata = test_data, type="response")
      results.sim$total_est[1] = sum(test_data$mod.pred) # calculate total (across years) estimated bycatch, unobserved trips
      results.sim[1,col.2011:(col.2011+4)] <- group_by(test_data, year) %>% summarize(byyear=sum(mod.pred)) %>% pull(byyear)
    ...
    # save output
    saveRDS(results, file = paste0("results/byyear_gamnonlinear_",spp[i],"_",pct[h],".rds"))
  }
