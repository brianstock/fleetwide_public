...
# Oct 25, 2017
# May 20, 2018 - re-run VAST space + effort model bc effort as offset did terrible
#                 other models used effort as covariate, so here we do that for VAST
# Aug 5, 2018
#   try to fix non-bias-corrected VAST with effort as covariate (in Q matrix)

# source("/home/brian/Documents/Bycatch/fleetwide/analysis/6_run_vast_delta_eff.R")
...
# install_github("nwfsc-assess/geostatistical_delta-GLMM", ref="3.5.0") 
library(TMB)
library(VAST)
library(dplyr)
...
Version = "VAST_v4_2_0"
# Version = "VAST_v5_2_0" # TMB model fails to compile using the latest version of VAST... dunno why
...
for(h in 1:length(pct)) {
  ...
  for(i in 1:length(spp)) {
    ...
    for(j in 1:n_sims) {
      ...
      tmp <- data.frame(matrix(NA, ncol = length(yr.labs)+1, nrow = 1))
      colnames(tmp) <- c(yr.labs,"runtime")
      ...
      if(same_factor) { # fit models
        ...
        Q_ik = cbind( Q_ik, Data_Stack[,'ret_plus1'])
        ...
        Extrapolation_List = SpatialDeltaGLMM::Prepare_Extrapolation_Data_Fn( Region=Region, 
                              ...
                              zone=10) # VAST was incorrectly using UTM zone 30 without this...
        ...
        TmbData = Data_Fn("Version"=Version, "FieldConfig"=FieldConfig, "OverdispersionConfig"=OverdispersionConfig, 
                          ...
                          "a_i"=rep(1,nrow(Data_Geostat)), # areaswept = 1 for all i (put Effort into Q matrix)
        ...
        TmbList = Build_TMB_Fn("TmbData"=TmbData, "RunDir"=DateFile, "Version"=Version, "RhoConfig"=RhoConfig, 
                              ...
                              "TmbDir"="/home/brian/R/x86_64-redhat-linux-gnu-library/3.5/VAST/executables")
        Obj = TmbList[["Obj"]]
        ...
        #       bias.correct.control=list(sd=FALSE, split=NULL, nsplit=NULL, vars_to_correct="D_i"))
        Opt = TMBhelper::Optimize( obj=Obj, lower=TmbList[["Lower"]], upper=TmbList[["Upper"]], 
              getsd=TRUE, savedir=DateFile, bias.correct=FALSE, newtonsteps=1)
        Report = Obj$report()

# saveRDS(Opt, file = paste0("/home/brian/Documents/Bycatch/bias_correct_VAST_test/vast_Opt_memsave_",spp[i],"_",pct[h],"_sim",j,".rds"))
# rm(list=c("Opt"))
# saveRDS(Obj, file = paste0("/home/brian/Documents/Bycatch/bias_correct_VAST_test/vast_Obj_memsave_",spp[i],"_",pct[h],"_sim",j,".rds"))
# rm(list=c("Obj"))
# saveRDS(Report, file = paste0("/home/brian/Documents/Bycatch/bias_correct_VAST_test/vast_Report_memsave_",spp[i],"_",pct[h],"_sim",j,".rds"))      
# gc()
        ...
        etime <- Sys.time()
        results.sim$runtime[1] <- etime-stime
        
        # print to screen
        print(paste0("Sim ",j,", species ",i,", pred = ",results.sim$total_est[1],", true = ",results.sim$total_true[1]))

        # results.sim.nobiascor <- results.sim
        # # pred.ind <- which(Data_Stack$predTF==1)
        # Summary = summary( Opt$SD, "report" )
        # Which = which( rownames(Summary) =="D_i" )
        # Dhat_i = Summary[Which,3]
        # test_data$predicted = Dhat_i[ pred.ind ]
        # results.sim$total_est[1] = sum(test_data$predicted) 
        # results.sim[1,col.2011:(col.2011+4)] <- group_by(test_data, year) %>% summarize(byyear=sum(predicted)) %>% pull(byyear)

        rm(list=c("Opt","Obj","Report"))
        gc()

# saveRDS(results.sim, file=paste0("/home/brian/Documents/Bycatch/bias_correct_VAST_test/vast_results_biascor_",spp[i],"_",pct[h],"_sim",j,".rds"))
# saveRDS(results.sim.old, file=paste0("/home/brian/Documents/Bycatch/bias_correct_VAST_test/vast_results_",spp[i],"_",pct[h],"_sim",j,".rds"))
    ...
    # results.biascor <- rbind(results, results.sim)
    write.csv(results,file="tmpres.csv",quote=FALSE)    
    
    } # end j (sim)
    
    # save output
    saveRDS(results, file = paste0("/home/brian/Documents/Bycatch/fleetwide/results/byyear_vast_eff_",spp[i],"_",pct[h],".rds"))
    saveRDS(results, file = paste0("/home/brian/Dropbox/bycatch/fleetwide/byyear_vast_eff_",spp[i],"_",pct[h],".rds"))
    # saveRDS(results.biascor, file = paste0("/home/brian/Documents/Bycatch/fleetwide/results/byyear_vast_eff_biascor_",spp[i],"_",pct[h],".rds"))
    # saveRDS(results.biascor, file = paste0("/home/brian/Dropbox/bycatch/fleetwide/byyear_vast_eff_biascor_",spp[i],"_",pct[h],".rds"))
  } # end i (species)
