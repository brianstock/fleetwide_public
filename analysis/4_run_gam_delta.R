...
for(i in 1:length(spp)) {
  ...
  dat$bimonth = as.factor(dat$bimonth)
  ...
  for(h in 1:length(pct)) {
    ...
    for(j in 1:n_sims) {
      ...
      results.sim = data.frame(model = rep("gamdelta",4),
      ...
      mod_binomial = gam(presence ~ s(avg_lat, avg_long, k=50) + bimonth + year + depth_interval + logret, 
                      data=training_data, discrete=TRUE, control = list(maxit=500, epsilon = 1e-06), family="binomial")
      mod_positive = gam(response ~ s(avg_lat, avg_long, k=50) + bimonth + year + depth_interval + logret, 
                      data=training_data[which(training_data$response>0),],
                      discrete=TRUE, control = list(maxit=500, epsilon = 1e-06), family="Gamma")  
      test_data$gam.pred <- predict(mod_binomial, newdata = test_data, type="response") * predict(mod_positive, newdata = test_data, type="response") # , se.fit = TRUE
      ...
      mod_binomial = gam(presence ~ s(avg_lat, avg_long, k=50) + bimonth + year + depth_interval, 
                      data=training_data, discrete=TRUE, control = list(maxit=500, epsilon = 1e-06), family="binomial")
      mod_positive = gam(response ~ s(avg_lat, avg_long, k=50) + bimonth + year + depth_interval, 
                      data=training_data[which(training_data$response>0),],
                      discrete=TRUE, control = list(maxit=500, epsilon = 1e-06), family="Gamma")  
      test_data$gam.pred <- predict(mod_binomial, newdata = test_data, type="response") * predict(mod_positive, newdata = test_data, type="response") # , se.fit = TRUE
      ...
      mod_binomial = gam(presence ~ bimonth + year + depth_interval + logret, 
                      data=training_data, discrete=TRUE, control = list(maxit=500, epsilon = 1e-06), family="binomial")
      mod_positive = gam(response ~ bimonth + year + depth_interval + logret, 
                      data=training_data[which(training_data$response>0),],
                      discrete=TRUE, control = list(maxit=500, epsilon = 1e-06), family="Gamma")  
      test_data$gam.pred <- predict(mod_binomial, newdata = test_data, type="response") * predict(mod_positive, newdata = test_data, type="response") # , se.fit = TRUE
      ...
      mod_binomial = gam(presence ~ bimonth + year + depth_interval, 
                      data=training_data, discrete=TRUE, control = list(maxit=500, epsilon = 1e-06), family="binomial")
      mod_positive = gam(response ~ bimonth + year + depth_interval, 
                      data=training_data[which(training_data$response>0),],
                      discrete=TRUE, control = list(maxit=500, epsilon = 1e-06), family="Gamma")  
      test_data$gam.pred <- predict(mod_binomial, newdata = test_data, type="response") * predict(mod_positive, newdata = test_data, type="response") # , se.fit = TRUE
    ...
    # save output
    saveRDS(results, file = paste0("results/byyear_gamdelta_",spp[i],"_",pct[h],".rds"))
  }
