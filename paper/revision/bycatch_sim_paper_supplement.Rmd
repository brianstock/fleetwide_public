...
author: Brian C. Stock^1^, Eric J. Ward^2^, James T. Thorson^2^, Jason E. Jannot^2^,
  Brice X. Semmens^1^
date: ''
output:
  pdf_document:
...
```{r covariate-effects, echo=FALSE, message=FALSE, warnings=FALSE, out.width='7in',fig.height=6, fig.width=7, fig.align = "center", fig.cap="Change in predictive performance (normalized RMSE) when adding fishing effort and spatial location as covariates in each model. For many species, adding space to the GAM-Delta and GAM-Total models led to worse predictions (positive change in RMSE, above dashed line). On the other hand, adding space to the RF-Delta model consistently improved predictions (negative change in RMSE, below dashed line). For RF-Total, including space had either slightly improved predictions or had no effect. Adding effort had little effect for nearly all species and models, and never had a larger effect than adding space.", fig.scap="Change in predictive performance (normalized RMSE) when adding fishing effort and spatial location as covariates in each model."}
...
dat$model <- factor(dat$model, levels = c("Ratio", "GAM Delta", "GAM", "RF Delta","RF"))
levels(dat$model) <- c("Ratio","GAM Delta","GAM Total","RF Delta","RF Total")
...
# dat <- dat %>% filter(model %in% c("Ratio", "GAM", "GAM Nonlinear","RF","RF Nonlinear")) %>% 
#               mutate(rmse = ifelse(rmse > 10, NA, rmse))
dat.allspecies <- dat
...
levels(dat.plot$model) <- c("Ratio","GAM Total","RF Total")
# dat.plot$model <- factor(dat.plot$model, levels = c("Ratio", "GAM", "GAM Nonlinear","RF","RF Nonlinear"))
# levels(dat.plot$model) <- c("Ratio","GAM Total","GAM Nonlinear","RF Total","RF Nonlinear")
